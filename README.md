...
- WPF : 디자인적으로 좀 더 뛰어난 윈앱 개발
- 순서 
     1. 언어, 플랫폼, 프로젝트 선택
        - 언어 : C#
        - 플랫폼 : 모든 플랫폼
        - 프로젝트 : 데스크톱, 웹, 콘솔
    2. Windows Forms 앱 선택 (.NET Framework가 적히지 않은 것)
    3. 프로젝트 명 입력
    4. 추가 정보
        - 프레임워크 : .NET 8.0(장기지원) 선택
        - LTS(Long Term Support) : 개발자가 기능, 보안 업데이트를 지원 - 안정성 높음
...
5. 보기 > 도구 상자 클릭
    ...
    - 보기 > 클래스 뷰
        - 컨트롤 보는 법
    - 더블클릭, 클릭으로 선택 후 폼에서 드래그 등 다양한 방법
...
11. 오류 발생
    ...
    - 이벤트 이름은 생성되고, 이벤트 정의 메소드는 생성되지 않았기 때문

    <img src="./image/cs0002.png" width="600">

    - FrmMain.Designer.cs 파일 오픈 후 오류난 이벤트 이름 삭제
    - #region Windows Form Designer generated code 확장
    - 오류난 이벤트 이름 삭제
        - BtnMsg.Click += ...
        - BtnOk.Click += ...
...
- `Button` : 마우스 클릭 위한 컨트롤
    ...
    - Location : 폼상의 위치 (x, y)
    - Size : 버튼 크기 (w, h)
    - TabIndex : 실행 후 탭으로 포커스가 가는 순서(label은 focus 안감)
    - Text : 버튼 표시 글씨
    - Visible : 화면 표현 여부
...
### C# 문법
- 문법 순서 - C++, Java와 거의 동일. C++와 차이점만 비교
    1. 기본구조 및 주석 - [소스](./day02/Day02Study/WinFormsApp1/FrmMain.cs)
        - 네임스페이스, 클래스, 메서드
    ...
    2. 자료형과 변수, Nullable
        ...
        - Nullable : 특수 타입 데이터타입 뒤에 ? 추가. null 할당 가능
        - var 타입 : 지역변수에서 타입을 동적으로 지정. 컴파일 시 해당 타입으로 자동 지정
            - 지역변수만 가능(전역변수 사용불가)
