...
- 기본분법
    ...
    2. 예외처리
        ...
        https://github.com/user-attachments/assets/72da4efe-1475-4c7a-afb5-0612523a8824
...
- `StatusStrip` : 상태표시줄 컨트롤. 서브 컨트롤 추가 가능
    ...
    - `StatusLabel`    : 텍스트만 표시, Lbl~
    - `ProgressBar`    : 진행 상황 표시, Prg~
    - `DropDownButton` : 드롭다운버튼, 이벤트 추가
    - `SplitButton`    : 버튼 + 드롭다운 결합형
    - `ControlHost`    : 다른 컨트롤을 끼워넣을 때 사용
    - `SizingGrip`     : 오른쪽 하단의 사이즈 조절 아이콘 표시

### VS 사용팁
- 소스코드 들여쓰기 문제 발생 시 : 편집 > 고급 > 문서서식 클릭

### C# 문법
- 기본 문법
    1. 델리게이트, 이벤트 [소스](./day05/Day05Study/SyntaxWinApp01/FrmMain.cs)
        - 메서드를 변수처럼 저장하고 호출할 수 있는 타입
            - C++ 함수포인터, 파이썬 콜백함수
        - 대리자는 실행할 메서드를 들고 있고, 대리자를 호출하면 대리자가 메서드를 대신 호출해줌
        - 윈폼에서 버튼을 클릭하면, 직접 메서드를 호출하는게 아니고, C#이 사용자의 움직임을 체크하고 있다가 클릭 `이벤트가 발생하면 그에 해당하는 메서드를 대신 실행(호출)` 시켜주는 것
        - 이벤트도 Delegate

        - 대리자의 장점
            - 메서드를 변수처럼 저장
            - 런타임(프로그램 실행) 시 메서드를 바꿀 수 있음
            - 하나의 대리자가 여러개의 메서드를 호출 할 수 있음

        - Action, Func, Predicate
            - 대리자 생략형
        - 이벤트 : 대리자의 특별한 형태. 어떤 일이 발생하면 연결된 메서드들 호출하는 것
        ...
        - Form 오픈 이벤트 및 생성자
            ...
            3. FrmMain_Activated 이벤트 : 폼이 활성화(바탕화면 가장 위에서 동작하는 상태)
            4. FrmMain_Shown 이벤트 : 폼이 바탕화면에 그려질 때 이벤트

        - Form 클로즈 이벤트
            - FrmMain_FormClosing 이벤트
            - FrmMain_FormClosed 이벤트
...
- 고급 문법
    ...
    5. 제네릭 : Generic
        ...
        ```
        - 위 3개의 메서드를 아래의 하나의 메서드로 통합한 코드
    ...
    6. 비동기, 스레드 [소스](./day05/Day05Study/SyntaxWinApp03/FrmMain.cs)
        ...
        - 콘솔에는 실시간 출력, 텍스트박스, 프로그레스바 등 컨트롤 표시가 실시간으로 되지 않음

        - 첫번째 해결방법
            - Application.DoEvents() 메서드 추가. 권장X
        - `두번째` 해결방법 : 같이 사용해야함
            - async : 비동기 프로그래밍 키워드 , await 키워드로 해결
            - 응답없음 나타나지 않음
        - 세번째 해결방법
            - 전통적인 스레드 사용. 권장X
        - `네번째` 해결방법
