namespace SyntaxWinApp02
{
    public partial class FrmMain : Form
    {
        ...
        private void BtnCheck_Click(object sender, EventArgs e)
        {
            ...
            TxtLog.Text += $"전통 짝수 리스트 : {string.Join(" ", resList)}\r\n";

            resList.Sort();

            TxtLog.Text += $"전통 정렬 리스트 : {string.Join(" ", resList)}\r\n";

            // 기본 LINQ 방식 > 3줄로 전통 방식 처리
            numbers = [14, 20, 11, 15, 18, 19, 16, 13, 12, 17];
            var resList2 = from n in numbers
                           where n % 2 == 0
                           orderby n
                           select n;
            TxtLog.Text += "LINQ 정렬 리스트 > " + string.Join(" ", resList2) + "\r\n";
        ...
        {
            // UI 초기화
            TxtTest.PlaceholderText = "테스트입니다.";    // 텍스트박스 예시 문구
            TxtTest.Size = new Size(200, 23);
            TxtTest.KeyPress += TxtTest_KeyPress; // Designer에서 작업하는 것과 동일
            TxtTest.Font = new Font("휴먼둥근헤드라인", 12, FontStyle.Italic);
        }
    ...
    public class Box<T> // where T : struct
    {
        ...
        public void Show()
        {
            Console.WriteLine($"Box의 값 : {Value}");
